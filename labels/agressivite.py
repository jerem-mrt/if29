...
tweets = db.tweet.find().limit(9999)
...
for profile in profiles:
    ...
    if len(timestamp) > 1:
        ...
        if diffTime > 1 :
            ...
            profiles[profile]['frequenceFriends'] = (followee[0] - followee[len(followee)-1]) / diffTime
        else :
            profiles[profile]['frequenceTweet'] = len(timestamp)
            profiles[profile]['frequenceFriends'] = (followee[0] - followee[len(followee)-1])
        
    profiles[profile]['agressivity'] = (profiles[profile]['frequenceFriends'] + profiles[profile]['frequenceTweet'])/350
for profile in profiles :
    if not db.essai.find_one({"_id": profile}):
        db.essai.insert_one({"_id" : profile, "frequenceFriends" : profiles[profile]['frequenceFriends'], "frequenceTweet" : profiles[profile]['frequenceTweet'], "agressivity" : profiles[profile]['agressivity']})
    else :
        db.essai.update_one({"_id" : profile},{"$set": {"frequenceFriends" : profiles[profile]['frequenceFriends'],"frequenceTweet" : profiles[profile]['frequenceTweet'], "agressivity" : profiles[profile]['agressivity']}})
