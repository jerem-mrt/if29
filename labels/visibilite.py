...
for y in stock :
    if not essai.find_one({"_id": y}):
        print(utilisateur)
        db.essai.insert_one({"_id" : y, "tag_tweet" : stock[y]['tag_tweet'], "mention_tweet" : stock[y]['mention_tweet'], "visibilite_moy" : stock[y]['visibilite_moy']})
    else :
        db.essai.update_one({"_id" : y},{"$set": {"tag_tweet" : stock[y]['tag_tweet'], "mention_tweet" : stock[y]['mention_tweet'], "visibilite_moy" : stock[y]['visibilite_moy']}})
